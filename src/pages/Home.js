...
export default function Home() {
  // Temporary state for the dropdowns (allows users to pick before confirming)
  const [tempYear, setTempYear] = useState(2023);
  const [tempEvent, setTempEvent] = useState("");
  const [tempSession, setTempSession] = useState("R");

  // Fetch event list based on the selected year
  const { events, loading, error } = useEvents(tempYear);

  // The final configuration that actually triggers data fetching in CoreContainer
  ...
  // Automatically select the first available event when the year changes or events load
  useEffect(() => {
    if (events.length > 0) {
  ...
  // Syncs the temporary dropdown state to the applied configuration
  const handleSubmit = () => {
    setAppliedConfig({
      year: tempYear,
      eventName: tempEvent,
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-4 font-sans">
      <div className="w-[90%] mx-auto space-y-4">
        ...
        {/* Header Section: Title and Current Active Stream display */}
        ...
        {/* Filters Card: Controls for Year, Event, and Session */}
        <div className="bg-slate-900 rounded-xl border border-slate-800 p-4 shadow-lg">
          <div className="flex flex-col md:flex-row gap-4 items-end">
            <div className="flex-1 w-full grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              {/* Event/Grand Prix Selection (Depends on Year) */}
              <div>
        ...
        {/* Main Content Area: Displays the Telemetry Charts/Data */}
